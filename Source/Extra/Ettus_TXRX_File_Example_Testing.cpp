    ...
    double settling_time)
{
    ...
    uhd::stream_args_t stream_args("fc32", "sc16"); // CPU, OTW.
    ...
    while (num_requested_samples > totalReceivedSamples)
    {
        ...
        // Error handling.
        if (rxMD.error_code == uhd::rx_metadata_t::ERROR_CODE_TIMEOUT) { std::cout << boost::format("Timeout while streaming") << std::endl; break; }
        if (rxMD.error_code == uhd::rx_metadata_t::ERROR_CODE_OVERFLOW) { std::cout << boost::format("Overflow while streaming") << std::endl; break; }
        if (rxMD.error_code != uhd::rx_metadata_t::ERROR_CODE_NONE) { throw std::runtime_error(str(boost::format("Receiver error %s") % rxMD.strerror())); }
...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    const wave_table_class wave_table("SINE", ampl);
    ...
    // ----------------------------------------------------------------------------------------------------- //
    // Use own tx worker.
    std::vector<std::complex<float>> txBuffer(20400);
    for (size_t n = 0; n < buff.size(); n++) 
    {
        buff[n] = wave_table(index += step);
    }
    std::thread transmit_thread([&]() 
        {
        transmitBuffer(txBuffer, tx_stream, md);
    });
    // Use my own receive function.
    total_num_samps = 2 * rx_rate;
    receiveBufferToFile(rx_usrp, "B210_SAMPLES_Example_Testing.bin", spb, total_num_samps, settling);
    // ----------------------------------------------------------------------------------------------------- //
