...
int UHD_SAFE_MAIN(int argc, char* argv[])
{
    ...
    double tx_rate_def, rx_rate_def, rate;
    rate = 1e6;
    tx_rate_def = rate;
    rx_rate_def = rate;
    ...
    double tx_freq_def, rx_freq_def, freq;
    freq = 1.1e9;
    tx_freq_def = freq;
    rx_freq_def = freq;
    ...
    desc.add_options()
        ...
        ("file", po::value<std::string>(&file)->default_value("Data/usrp_samples.dat"), "name of the file to write binary samples to")
        ...
        ("rx_rate", po::value<double>(&rx_rate)->default_value(rx_rate_def), "specify to perform a RX rate test (sps)")
        ("tx_rate", po::value<double>(&tx_rate)->default_value(tx_rate_def), "specify to perform a TX rate test (sps)")

        // ***********************************************************************

        ("tx-freq", po::value<double>(&tx_freq)->default_value(tx_freq_def), "transmit RF center frequency in Hz")
        ("rx-freq", po::value<double>(&rx_freq)->default_value(rx_freq_def), "receive RF center frequency in Hz")
